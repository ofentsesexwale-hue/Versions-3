...
ATLAS_FORMS = {
    ...
    'monthly_report': {
        'title': 'C06 Monthly Household Services Report',
        'official_title': 'C06: Monthly Household Services Report (v2.0)',
        'header': 'ccg',
        'orientation': 'landscape',
        'keywords': ['C06', 'MONTHLY HOUSEHOLD SERVICES', 'CCG FORM'],
        'checklist_item': ('monthly_report', 'Monthly Household Services Report - C06'),
        'geometry': 'word',
        'header_only': True,
    },
    'educational': {
        'title': 'Educational Progress Record',
        'official_title': 'Educational Progress Record',
        'header': 'dsd',
        'orientation': 'landscape',
        'keywords': ['EDUCATIONAL PROGRESS', 'FAMILY NAME', 'TERM'],
        'checklist_item': ('school_report', 'Educational Plan'),
        'geometry': 'word',
        'header_only': True,
    },
    'site_visit': {
        'title': 'Site Visit Form',
        'official_title': 'Site Visit form',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['SITE VISIT', 'NAME OF CARE GIVER', 'PURPOSE OF THE VISIT'],
        'checklist_item': ('process_note', 'SAW Process note - CW 11'),
        'geometry': 'word',
        'identity_only': True,
    },
    'exit': {
        'title': 'Family Exit Form',
        'official_title': 'Family Exit form',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['FAMILY EXIT', 'CYCW', 'OUTCOME OF THE VISIT'],
        'checklist_item': ('process_note', 'Termination report / Exit forms - CW 13'),
        'geometry': 'word',
        'identity_only': True,
    },
    'checklist': {
        'title': 'NPO Check List',
        'official_title': 'CASE MANAGEMENT: NPO CHECK LIST',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['NPO CHECK LIST', 'CASE MANAGEMENT', 'IS THERE EVIDENCE'],
        'checklist_item': ('intake_form', 'C01'),
        'geometry': 'word',
        'header_only': True,
    },
    'content_page': {
        'title': 'Content Page',
        'official_title': 'Content Page',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['CONTENT PAGE', 'HOUSEHOLD NUMBER', 'INTAKE FORMS'],
        'checklist_item': ('intake_form', 'C01'),
        'geometry': 'word',
        'header_only': True,
    },
    ...
    'termination': {
        'title': 'CW 13: Termination Report',
        'official_title': 'CW 13: Termination Report',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['CW 13', 'CW13', 'TERMINATION', 'CLIENT SURNAME'],
        'checklist_item': ('process_note', 'Termination report / Exit forms - CW 13'),
        'geometry': 'word',
        'identity_only': True,
    },
    'internal_referral': {
        'title': 'CW 4a: Internal Referral',
        'official_title': 'CW 4a: Internal Referral Form',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['CW 4A', 'CW4A', 'INTERNAL REFERRAL', 'CASE REFERRED FROM'],
        'checklist_item': ('referral_form', 'Referral form'),
        'geometry': 'word',
        'identity_only': True,
    },
    'referral': {
        'title': 'CW 4b: External Referral',
        'official_title': 'CW 4b: External Referral Form',
        'header': 'dsd',
        'orientation': 'portrait',
        'keywords': ['CW 4B', 'CW4B', 'EXTERNAL REFERRAL', 'CASE REFERRED TO'],
        'checklist_item': ('referral_form', 'Referral form'),
        'geometry': 'word',
        'identity_only': True,
    },
...
def _build_fields():
    ...
    monthly_report = [
        _f('__display.personnel', 'CCG Name', 0,
           _word_cell((0.1200, 0.0700, 0.4200, 0.1000)), 'printed'),
    ]
    educational = [
        _f('caregiver.surname', 'Family name', 0,
           _word_cell((0.1400, 0.0700, 0.4500, 0.1000)), 'handwrite'),
    ]
    site_visit = [
        _f('caregiver.name', 'Name of care giver', 0,
           _word_cell((0.2800, 0.1000, 0.9000, 0.1250)), 'handwrite'),
        _f('caregiver.surname', 'Family', 0,
           _word_cell((0.1600, 0.1300, 0.9000, 0.1550)), 'handwrite'),
    ]
    exit_form = [
        _f('caregiver.surname', 'Family Name', 0,
           _word_cell((0.2200, 0.1000, 0.9000, 0.1250)), 'handwrite'),
        _f('household.date_registered', 'Registration Date', 0,
           _word_cell((0.2800, 0.1900, 0.9000, 0.2150)), 'date'),
    ]
    checklist = [
        _f('__display.organisation', 'Name of the Organisation', 0,
           _word_cell((0.3200, 0.0950, 0.9200, 0.1200)), 'printed'),
        _f('__display.personnel', "Practitioner's name and surname", 0,
           _word_cell((0.3800, 0.1250, 0.9200, 0.1500)), 'printed'),
    ]
    content_page = [
        _f('household.org_household_number', 'Household Number', 0,
           _word_cell((0.2800, 0.0900, 0.7000, 0.1200)), 'printed'),
    ]
    termination = [
        _f('caregiver.surname', 'Client Surname', 0,
           _word_cell((0.0600, 0.1200, 0.2800, 0.1500)), 'handwrite'),
        _f('caregiver.name', 'Client First name', 0,
           _word_cell((0.3000, 0.1200, 0.5200, 0.1500)), 'handwrite'),
        _f('caregiver.id_number', 'Client ID Number', 0,
           _word_cell((0.5400, 0.1200, 0.7600, 0.1500)), 'printed'),
        _f('household.org_household_number', 'File No', 0,
           _word_cell((0.7800, 0.1200, 0.9400, 0.1500)), 'printed'),
    ]
    internal_referral = [
        _f('household.org_household_number', 'DSD Reference Number', 0,
           _word_cell((0.2800, 0.2200, 0.4800, 0.2450)), 'printed'),
        _f('member.0.id_number', 'Identity no', 0,
           _word_cell((0.6200, 0.2200, 0.9200, 0.2450)), 'printed'),
        _f('member.0.name', 'Client name', 0,
           _word_cell((0.2200, 0.2500, 0.4800, 0.2750)), 'handwrite'),
    ]
    referral = [
        _f('member.0.name', 'Client name', 0,
           _word_cell((0.2200, 0.2800, 0.4800, 0.3050)), 'handwrite'),
        _f('household.org_household_number', 'DSD Reference Number', 0,
           _word_cell((0.2800, 0.3100, 0.4800, 0.3350)), 'printed'),
        _f('member.0.id_number', 'Identity no', 0,
           _word_cell((0.6200, 0.3100, 0.9200, 0.3350)), 'printed'),
    ]
    return {
        'c01': c01,
        'intake': intake,
        ...
        'cow2_note': cow2,
        'process_note': [
            _f('process_note.client_surname', 'Client(s) Surname', 0,
               _word_cell((0.0600, 0.1100, 0.2800, 0.1400)), 'handwrite'),
            _f('process_note.client_first_name', 'Client(s) First name', 0,
               _word_cell((0.3000, 0.1100, 0.5200, 0.1400)), 'handwrite'),
            _f('process_note.client_id_number', 'Client(s) ID Number', 0,
               _word_cell((0.5400, 0.1100, 0.7600, 0.1400)), 'printed'),
            _f('process_note.file_no', 'File No', 0,
               _word_cell((0.7800, 0.1100, 0.9400, 0.1400)), 'printed'),
            _f('process_note.problem_code', 'Problem Code', 0,
               _word_cell((0.2800, 0.1600, 0.4800, 0.1900)), 'printed'),
            _f('process_note.intervention_code', 'Intervention Code', 0,
               _word_cell((0.5200, 0.1600, 0.7200, 0.1900)), 'printed'),
            _f('process_note.type_of_engagement', 'Office', 0,
               _word_cell((0.2800, 0.1950, 0.3100, 0.2150)), 'checkbox',
               option='Office', group='process_note.type_of_engagement'),
            _f('process_note.type_of_engagement', 'Home', 0,
               _word_cell((0.3400, 0.1950, 0.3700, 0.2150)), 'checkbox',
               option='Home', group='process_note.type_of_engagement'),
            _f('process_note.type_of_engagement', 'School', 0,
               _word_cell((0.4000, 0.1950, 0.4300, 0.2150)), 'checkbox',
               option='School', group='process_note.type_of_engagement'),
            _f('process_note.type_of_engagement', 'Court', 0,
               _word_cell((0.5000, 0.1950, 0.5300, 0.2150)), 'checkbox',
               option='Court', group='process_note.type_of_engagement'),
            _f('process_note.type_of_engagement', 'Telephone', 0,
               _word_cell((0.6000, 0.1950, 0.6300, 0.2150)), 'checkbox',
               option='Telephone', group='process_note.type_of_engagement'),
            _f('process_note.type_of_engagement', 'Other', 0,
               _word_cell((0.7200, 0.1950, 0.7500, 0.2150)), 'checkbox',
               option='Other', group='process_note.type_of_engagement'),
            _f('process_note.purpose_and_what_transpired', 'Purpose of engagement and what transpired', 0,
               _word_cell((0.0600, 0.2400, 0.9400, 0.4200)), 'narrative'),
        ],
    }
